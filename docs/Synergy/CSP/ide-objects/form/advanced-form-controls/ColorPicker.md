...
Renk seçimi yapılarak seçilen rengin formda bir yerde kullanılması gerektiğinde kullanılan nesnedir.
...
`Value` - Nesnede bir rengin otomatik olarak gelmesi istenirse belirlendiği alandır.

`Default Color` - Nesnede varsayılan rengin ayarlandığı alandır. Value ve Default Color alanlarında seçim yapıldıysa Default Color alanındaki seçim nesnede gösterilecektir.

`Preset Colors` - Nesne içinde hızlı renk seçimi alanında gösterilecek renklerin belirlendiği alandır. 
